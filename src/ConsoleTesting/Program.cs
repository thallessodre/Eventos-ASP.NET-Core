...
namespace ConsoleTesting
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Registro com erros
            cmd = new RegistrarEventoCommand("DevX", DateTime.Now.AddDays(2), DateTime.Now.AddDays(1), false, 0, false, "");
            Inicio(cmd);
            bus.SendCommand(cmd);
            Fim(cmd);

            //Atualizar evento
            var cmd2 = new AtualizarEventoCommand(Guid.NewGuid(), "DevX2", "", "", DateTime.Now.AddDays(1), DateTime.Now.AddDays(2), true, 0, true, "Empresa2");
            Inicio(cmd2);
